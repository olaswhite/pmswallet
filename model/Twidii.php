...
public function __construct($db){
	...
	function checkUser($email){
        ...
        function userReg($username,$email,$phoneno,$password){


				$stmt = $this->conn->prepare( "INSERT INTO twiidi SET username=:username, email=:email, 
		...
		function updateTrans($ref, $username,$to,$amount){
            ...
            function createWithdrawWallet($username,$accountNumber,$amount){


				$query= " INSERT  withdraw_wallet SET balance = balance, accountnumber=:accountnumber WHERE username= :username";
				$stmt = $this->conn->prepare($query);

				$accountNumber= htmlspecialchars(strip_tags($accountNumber));
	
				$amount= htmlspecialchars(strip_tags($amount));
				
				$stmt->bindParam(":accountname", $accountNumber);
				$stmt->bindParam(":balance", $amount);
				$stmt->bindParam(":username", $username);
        ...
        function tranHistory($username){
			...
			$query= "SELECT * FROM trans WHERE username=?";
			$stmt = $this->conn->prepare($query);
			$stmt->execute([$username]);			
				return $stmt;
		}
